...
selectedcatg = ['Geslacht','vrgeschiedenis_myochardinfarct','vrgeschiedenis_PCI','vrgeschiedenis_CABG','vrgeschiedenis_CVA_TIA','vrgeschiedenis_vaatlijden','vrgeschiedenis_hartfalen','vrgeschiedenis_maligniteit','vrgeschiedenis_COPD','vrgeschiedenis_atriumfibrilleren','TIA','CVA_Niet_Bloedig','CVA_Bloedig','LV_Functie','dialyse','riscf_roken','riscf_familieanamnese','riscf_hypertensie','riscf_hypercholesterolemie','riscf_diabetes','roken','ECG_Ritme','Radialis','Femoralis','Brachialis','vd_1','vd_2','vd_3']
selectedcong = ['Geboortedatum', 'lengte','gewicht','bloeddruk','HB','HT','INR','Glucose','Kreat','Trombocyten','Leukocyten','Cholesterol_totaal','Cholesterol_ldl']
selectedtarget = 'lbl'
shapenn = 0
...
def initforcnn(selectedcon, selectedcat):
	...
	indexmax = len(dftest.columns) -1
	#set parameters for the CNN
	cnntrain = dftrain[dftrain.columns.difference([indexmax])]
	cnntest = dftest[dftest.columns.difference([indexmax])]
	cnnlbltrain = dftrain[indexmax].values
	cnnlbltest = dftest[indexmax].values
	return cnntrain, cnntest, cnnlbltrain, cnnlbltest, indexmax
...
def randomprod(data):
	templist = random.sample(data, 35)
	return templist
...
def main():
	selectedheaders = randomprod(selectedcong + selectedcatg)
	selectedcon = list(set(selectedcong) & set(selectedheaders))
	selectedcat = list(set(selectedcatg) & set(selectedheaders))

	trainsetcnn, testsetcnn, trainlblcnn, testlblcnn, shape = initforcnn(selectedcon, selectedcat)
	editshape(shape)
	...
	with open("results.txt", "a") as file:
		file.write(str(eval_results["accuracy"])+' '.join(selectedcon)+' '.join(selectedcat)+'\n')
for i in range(100):
	main()
