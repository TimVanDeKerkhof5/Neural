...
def dnn():
	training_set = tf.contrib.learn.datasets.base.load_csv_without_header(filename='train.csv',target_dtype=np.int,features_dtype=np.float64)
	test_set = tf.contrib.learn.datasets.base.load_csv_without_header(filename='test.csv',target_dtype=np.int,features_dtype=np.float64)
	print(test_set)
	feature_columns = [tf.feature_column.numeric_column("x",shape=[97])]

	classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns,hidden_units=[30,40,30],model_dir="/tmp/read_model")
